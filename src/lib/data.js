...
const sheetUrl = "https://docs.google.com/spreadsheets/d/e/2PACX-1vT-tMtWQ4WECLecb4dsPLqxuwg-tb7_Zn38BAd85Jgxm4F8mfpkZSu5BByDwmGo-dK7DYiJITP7z9A6/pub?gid=0&single=true&output=csv";
...
d3.csv(sheetUrl).then(data => {
  const structured = data.map(row => {
    const element = {
      ...
      ypos: +row.ypos,
      series: row.series,
      important_element: row.usgs_critical_mineral === "1" || row.doe_critical_mineral === "1" || row.elements_associated_with_critical_minerals === "1",
      ...
      elements_associated_with_critical_minerals: row.elements_associated_with_critical_minerals === "1",
